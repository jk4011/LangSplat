...
# ============================================================
# LangSplat Full Experiment: Timing + Evaluation
# Datasets: LeRF (4), 3D-OVS (10), DL3DV (4)
# GPU: 4 only
# ============================================================
...
# ============================================================
# Helper functions (using awk instead of bc)
# ============================================================
calc() {
...
# ============================================================
# Pipeline: LeRF / 3D-OVS (no preprocess)
# ============================================================
run_scene_no_preprocess() {
    local DBASE=$1
    local SCENE=$2
    local DATASET=$3      # "lerf" or "3d_ovs"
    local DATASET_LABEL=$4 # "LeRF" or "3D-OVS"
    ...
    # --- Step 3: RGB 3DGS Training ---
    echo "[${SCENE}] Step 3: RGB 3DGS Training (30K iterations)"
    ...
    echo "[${SCENE}] Timing: RECON=${RECONSTRUCTION_TIME}s LIFTING=${LIFTING_3D}s AE_TRAIN=${AE_TRAIN_TIME}s TOTAL=${TOTAL}s"

    # --- Step 5: Render ---
    echo "[${SCENE}] Step 5: Rendering (3 levels)"
    for level in 1 2 3; do
        $PYTHON $BASE/render.py \
            -s $SCENE_PATH \
            -m ${MODEL_BASE}_${level} \
            --include_feature --skip_test \
            2>&1 | tee $SCENE_LOG/render_${SCENE}_level${level}.log
    done

    # --- Step 6: Setup eval symlinks + Evaluate ---
    ...
    if [ "$DATASET" = "lerf" ]; then
        ...
        $PYTHON evaluate_iou_loc.py \
            --dataset_name $SCENE \
            --feat_dir $OUT/$DATASET \
            --ae_ckpt_dir $CKPTS \
            --output_dir $EVAL_OUT/$DATASET \
            --json_folder $LERF_LABEL \
            --mask_thresh 0.4 \
        ...
        if [ -n "$EVAL_LOG" ]; then
            EVAL_RESULT=$(extract_iou_loc "$EVAL_LOG")
        else
            EVAL_RESULT=$(extract_iou_loc "$SCENE_LOG/eval_${SCENE}.log")
        fi
        ...
        EVAL_LOG=$(ls -t $EVAL_OUT/$DATASET/$SCENE/*.log 2>/dev/null | head -1)
        if [ -n "$EVAL_LOG" ]; then
            MIOU=$(extract_miou_3dovs "$EVAL_LOG")
        else
            MIOU=$(extract_miou_3dovs "$SCENE_LOG/eval_${SCENE}.log")
        fi
...
# ============================================================
# Pipeline: DL3DV (with preprocess)
# ============================================================
run_scene_dl3dv() {
    local SCENE=$1
    ...
    echo "[${SCENE}] Step 3: Autoencoder Test (encode to dim3)"
    cd $BASE/autoencoder
    $PYTHON test.py \
        --dataset_path $SCENE_PATH \
        --dataset_name ${SCENE}_exp \
    ...
    # --- Step 4: RGB 3DGS Training ---
    echo "[${SCENE}] Step 4: RGB 3DGS Training (30K iterations)"
    $PYTHON $BASE/train.py \
        -s $SCENE_PATH \
        -m $MODEL_BASE \
        --port $PORT \
    ...
    echo "[${SCENE}] Timing: SAM=${SAM_TIME}s RECON=${RECONSTRUCTION_TIME}s LIFTING=${LIFTING_3D}s CLIP=${CLIP_TIME}s AE_TRAIN=${AE_TRAIN_TIME}s TOTAL=${TOTAL}s"

    # --- Step 6: Render ---
    echo "[${SCENE}] Step 6: Rendering (3 levels)"
    for level in 1 2 3; do
        $PYTHON $BASE/render.py \
            -s $SCENE_PATH \
            -m ${MODEL_BASE}_${level} \
...
# Run all scenes
# ============================================================
echo "===== LangSplat Full Experiment ====="
echo "GPU: $CUDA_VISIBLE_DEVICES"
echo "Start time: $(date)"
echo ""

# LeRF scenes
echo "========== LeRF Dataset (4 scenes) =========="
for scene in $LERF_SCENES; do
    run_scene_no_preprocess $LERF_BASE $scene "lerf" "LeRF"
done

# 3D-OVS scenes
echo "========== 3D-OVS Dataset (10 scenes) =========="
for scene in $OVS_SCENES; do
    run_scene_no_preprocess $OVS_BASE $scene "3d_ovs" "3D-OVS"
done
